...
class RDbAuthManager extends CDbAuthManager
{
	...
	//$itemTable = $this->db->quoteTableName($this->db->quoteTableName($this->itemTable));
	//$rightsTable = $this->db->quoteTableName($this->db->quoteTableName($this->rightsTable));
	//$itemChildTable = $this->db->quoteTableName($this->db->quoteTableName($this->itemChildTable));
	/**
	...
	*/
	...
	{
		$sql = "SELECT * FROM {$this->db->quoteTableName($this->assignmentTable)} WHERE itemname=:itemname";
		$command = $this->db->createCommand($sql);
	...
	*/
	public function updateItemWeight($result)
	{
		...
		{
			$sql = "SELECT COUNT(*) FROM {$this->db->quoteTableName($this->rightsTable)}
			...
			{
				$sql = "UPDATE {$this->db->quoteTableName($this->rightsTable)}
					SET weight=:weight
					WHERE itemname=:itemname";
				$command = $this->db->createCommand($sql);
			...
			else
			{
				...
				{
					$sql = "INSERT INTO {$this->db->quoteTableName($this->rightsTable)} (itemname, type, weight)
